...
export function MainWindow(props) {
  ...
  const handleJoinConfirm = async () => {
    const input = joinRoomCode().trim();
    if (!input) return;

    // Check if the user pasted a full Magic Link
    if (input.startsWith("visualtalk://join?")) {
      try {
        // Ask Rust to parse the URL and extract the token/url/room
        const parsedData = await invoke("parse_invite_link", { inviteLink: input });

        console.log("Parsed Magic Link:", parsedData);

        // Pass the rich data up to the parent/window spawner
        // We include the token and url so LiveKit knows how to connect!
        props.onJoinMeeting({
          room: parsedData.room,
          token: parsedData.token,
          url: parsedData.url,
          identity: "Guest" // Default name, or prompt for one if you want!
        });

        setShowJoinModal(false);
      } catch (err) {
        alert(`Invalid Invite Link: ${err}`);
      }
    } else {
      // Standard Room Code flow (e.g., user typed "general" or "room-7137")
      // Behaves exactly like your original code
      props.onJoinMeeting({
        room: input,
        token: null, // No token yet, Host will need to generate it or use PMI
        url: null,
  ...
  const handleGuestJoin = (joinData) => {
    // joinData looks like: { url: "wss://...", token: "eyJ...", room: "room-7137", identity: "Jane" }

    console.log("Guest joining with:", joinData);

    // TODO: Call your existing Tauri command to open the meeting webview window!
    // You probably already have something like `invoke('spawn_meeting_window', ...)` 
    // from our very first session together!

    // Example:
    // invoke('spawn_meeting_window', { 
    //   room: joinData.room, 
  ...
  return (
    <div class="flex h-screen bg-[#111111] text-white select-none overflow-hidden">
      ...
      <aside class="hidden md:flex w-16 bg-[#1c1c1c] flex-col items-center justify-between py-5 border-r border-gray-800/40 shrink-0">
        <div class="flex flex-col items-center space-y-5">
          <button
            ...
            class={`flex flex-col items-center transition group focus:outline-none ${activeTab() === "home"
              ? "text-blue-400"
              : "text-gray-400 hover:text-white"
              }`}
          >
            <div
              class={`p-1.5 rounded-xl mb-0.5 transition ${activeTab() === "home"
          ...
          <button
            ...
            class={`flex flex-col items-center transition group focus:outline-none ${activeTab() === "meetings"
              ? "text-blue-400"
              : "text-gray-400 hover:text-white"
              }`}
          >
            <div
              class={`p-1.5 rounded-xl mb-0.5 transition ${activeTab() === "meetings"
                ? "bg-blue-600/20 text-blue-400"
                : "text-gray-400 group-hover:bg-gray-800"
                }`}
            >
          ...
          <button
            ...
            class={`flex flex-col items-center transition group focus:outline-none ${activeTab() === "hub"
              ? "text-blue-400"
              : "text-gray-400 hover:text-white"
              }`}
          >
            <div
              class={`p-1.5 rounded-xl mb-0.5 transition ${activeTab() === "hub"
                ? "bg-blue-600/20 text-blue-400"
                : "text-gray-400 group-hover:bg-gray-800"
                }`}
            >
        ...
        <div class="flex flex-col items-center">
          <button
            ...
            class={`p-2 rounded-xl transition focus:outline-none ${activeTab() === "settings"
              ? "bg-blue-600/20 text-blue-400"
          ...
          <div class="max-w-xl mx-auto w-full my-4 px-3">
            ...
            <div class="grid grid-cols-4 gap-x-1 sm:gap-6 justify-items-center items-start">
              ...
              <button
                ...
                class={`flex flex-col items-center w-full !bg-transparent group border-none outline-none shadow-none p-0 ${activeTab() === "notes" ? "text-blue-400" : ""
                  }`}
              >
                <div class={`w-13 h-13 sm:w-16 sm:h-16 rounded-[20px] sm:rounded-[22px] flex items-center justify-center mb-1.5 shadow-md group-hover:scale-105 group-hover:-translate-y-1.5 transition-transform duration-200 ease-out ${activeTab() === "notes" ? "bg-blue-600" : "bg-[#0E71EB]"
                  }`}>
                    ...
                    {(m) => (
                      <div class="flex items-center justify-between p-3 rounded-lg bg-[#242424] hover:bg-[#2a2a2a] transition">
                        ...
                        <div class="flex items-center space-x-2">
                          <button
                            onClick={async () => {
                              await navigator.clipboard.writeText(m.invite_link);
                              alert("Invite link copied! Send this to participants.");
                            }}
                            class="px-3 py-1 bg-[#2a2a2a] hover:bg-[#3a3a3a] border border-gray-700 text-xs font-semibold rounded-md transition text-gray-200"
        ...
        <Show when={showJoinModal()}>
          ...
          >
            <div class="flex flex-col space-y-4">
              <div>
                <label class="block text-xs font-medium text-gray-400 mb-1.5">Invite Link or Room Code</label>
                <input
                  type="text"
                  value={joinRoomCode()}
                  onInput={(e) => setJoinRoomCode(e.currentTarget.value)}
                  class="w-full px-3 py-2 rounded-lg bg-[#111111] border border-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-600/50 font-mono text-sm"
                  placeholder="visualtalk://join?... OR room-7137"
                  onKeyDown={(e) => e.key === 'Enter' && handleJoinConfirm()} // Allow pressing Enter to join!
                />
                <p class="text-[10px] text-gray-500 mt-2">Paste an invite link from a host, or enter a room code directly.</p>
              </div>
            </div>
          </Modal>
        </Show>
      ...
      <div class="md:hidden fixed bottom-0 left-0 right-0 bg-[#1c1c1c] border-t border-gray-800/40 flex justify-around items-center py-2 z-50">
        <button
          onClick={() => setActiveTab("home")}
          class={`flex flex-col items-center p-2 rounded-lg transition ${activeTab() === "home" ? "text-blue-400" : "text-gray-400"
            }`}
        >
        ...
        <button
          onClick={() => setActiveTab("meetings")}
          class={`flex flex-col items-center p-2 rounded-lg transition ${activeTab() === "meetings" ? "text-blue-400" : "text-gray-400"
            }`}
        >
        ...
        <button
          onClick={() => setActiveTab("settings")}
          class={`flex flex-col items-center p-2 rounded-lg transition ${activeTab() === "settings" ? "text-blue-400" : "text-gray-400"
            }`}
        >
